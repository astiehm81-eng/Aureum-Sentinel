...
REFRESH_RATE = 300         
RUNTIME_LIMIT = 800        
MAX_WORKERS = 50 # Reduziert für stabilere GitHub-Leistung
...
def ticker_engine():
    ...
    force_log("🛡️ Engine A: Live-Puls gestartet.")
    ...
    while (time.time() - start_time) < RUNTIME_LIMIT:
        ...
        force_log(f"💓 HEARTBEAT - Überprüfe {len(pool)} Assets...")
        results = []
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as exe:
            futures = [exe.submit(process, a) for a in pool]
        ...
        force_log(f"✅ Puls beendet. Warte {REFRESH_RATE}s...")
